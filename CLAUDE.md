...
9. **Keep tests fast.** Unit and integration tests should be quick. Never use `setTimeout` / `sleep` to wait out real timers — use fake timers (`vi.useFakeTimers()` + `vi.advanceTimersByTime()`) instead. For loading-state tests, use a never-resolving handler (`new Promise<never>(() => {})`) rather than a timed delay.
10. **Use `userEvent.setup()` per test.** Always instantiate `const user = userEvent.setup()` at the top of each test rather than calling `userEvent.*` directly. The standalone API uses real pointer-event delays that slow tests down.
11. **No flaky patterns.** Never wait with raw `setTimeout` in tests. Use `waitFor()` with a specific assertion instead of arbitrary delays. When waiting for multiple async conditions, assert the precise observable state you need (e.g., check an `aria-haspopup` attribute) rather than sleeping past a race window.
12. **Prefer `fireEvent` over `userEvent` for non-interaction state changes.** Use `userEvent` for realistic click/type interactions. Use `fireEvent` for direct DOM events (e.g., `change` on a number input, `keyDown`) where the goal is to set state, not simulate the full user gesture.
13. **Do not assert on `debug` or `info` log calls.** Only assert on `warn` or `error` log calls when the log message is meaningful signal (e.g., a retry warning or a caught error worth surfacing). Internal debug/info logging is an implementation detail — testing it makes tests fragile and couples them to logging choices rather than behavior.
