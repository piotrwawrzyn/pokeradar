...
// Load shop config
const configPath = path.join(__dirname, '../../src/config/shops/xzone.json');
const config: ShopConfig = JSON.parse(fs.readFileSync(configPath, 'utf-8'));

// Load test fixture
const fixturePath = path.join(__dirname, '../fixtures/xzone.json');
const fixture = JSON.parse(fs.readFileSync(fixturePath, 'utf-8'));

test('xzone.pl integration tests', async (t) => {
