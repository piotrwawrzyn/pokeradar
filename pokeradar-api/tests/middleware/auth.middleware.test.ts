...
describe('resolveDbUser middleware', () => {
  it('creates a new MongoDB user on first authenticated request', async () => {
    const res = await request(app).get('/watchlist').set('Authorization', 'Bearer clerk_new_user');
  ...
  it('reuses existing MongoDB user on subsequent requests', async () => {
    ...
    await request(app).get('/watchlist').set('Authorization', 'Bearer clerk_existing');
  ...
  it('creates separate users for different Clerk IDs', async () => {
    await request(app).get('/watchlist').set('Authorization', 'Bearer clerk_user_a');

    await request(app).get('/watchlist').set('Authorization', 'Bearer clerk_user_b');
